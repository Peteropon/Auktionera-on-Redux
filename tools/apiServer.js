...
server.post("/auctions/", function (req, res, next) {
...
function validateAuction(auction) {
  if (!auction.title) return "Title is required.";
  if (!auction.user) return "Author is required.";
  if (!auction.category) return "Category is required.";
  return "";
}
